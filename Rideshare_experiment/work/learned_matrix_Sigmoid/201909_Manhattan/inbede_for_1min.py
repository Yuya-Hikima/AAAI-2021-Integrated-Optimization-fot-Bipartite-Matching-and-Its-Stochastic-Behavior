...
df_loc=pd.read_csv("../locationID_location_all.csv")
...
df = pd.read_csv("../green_tripdata_2019-09.csv",parse_dates=['lpep_pickup_datetime','lpep_dropoff_datetime'])
...
#gdf_ex_pu=df.values
df = pd.read_csv("../yellow_tripdata_2019-09.csv",parse_dates=['tpep_pickup_datetime','tpep_dropoff_datetime'])
df=pd.merge(df, df_loc, how="inner" ,left_on="PULocationID",right_on="LocationID")
df=df[(df["trip_distance"] >10**(-3))&(df["total_amount"] >10**(-3))&(df["borough"] == place)&(df["PULocationID"] < 264)&(df["DOLocationID"] < 264)]
df2=df[['borough','PULocationID', 'DOLocationID','trip_distance','total_amount','tpep_pickup_datetime','tpep_dropoff_datetime']]
#ydf_ex_pu=df.values
#df_ex_pu=np.concatenate([gdf_ex_pu, ydf_ex_pu])

# kmに変換
#df_ex_pu[:,0]=df_ex_pu[:,0]*1.60934
...
for day_tmp in range(30):
    ...
    for tt_tmp in range(120):
        ...
        if minute+1<60:
            after_1min=datetime.datetime(year,month,day,hour,minute+1,second)
        else:
            after_1min=datetime.datetime(year,month,day,hour+1,minute+1-60,second)


        df1_minu=df1_day[(df1_day["lpep_pickup_datetime"] > set_time)& (df1_day["lpep_pickup_datetime"] < after_1min)]
        df2_minu=df2_day[(df2_day["tpep_pickup_datetime"] > set_time)& (df2_day["tpep_pickup_datetime"] < after_1min)]

        sdf1_minu=df1_day[(df1_day["lpep_dropoff_datetime"] > set_time)& (df1_day["lpep_dropoff_datetime"] < after_1min)]
        sdf2_minu=df2_day[(df2_day["tpep_dropoff_datetime"] > set_time)& (df2_day["tpep_dropoff_datetime"] < after_1min)]
        ...
        #df_ex_do[:,3]=df_ex_do[:,3]*1.60934  #別に使わない
        ...
        Pr=1-1/(1+np.exp(((-P+1.3*df_ex_pu[:,4])/((0.3*np.sqrt(np.float(3))/np.pi)*df_ex_pu[:,4])).astype(np.float64)))
        reward=np.zeros(n)
        ...
        #X=np.hstack([req_pu_loc_x,req_pu_loc_y,df_ex_pu[:,4:7]])
        ...
        for i in range(df_ex_pu.shape[0]):
            ...
            #D_a=np.vstack([D_a,X])
            #c_a=np.vstack([c_a,reward[i]])
            #with open('D_a.csv', mode='a') as f:
            #    writer = csv.writer(f)
            #    writer.writerow(X)

            #with open('c_a.csv', mode='a') as f:
            #    writer = csv.writer(f)
            #    writer.writerow([reward[i],0])
...
with open('A_0_09', 'wb') as web:
  pickle.dump(A_0, web)
with open('b_0_09', 'wb') as web:
  pickle.dump(b_0, web)
with open('A_1_09', 'wb') as web:
  pickle.dump(A_1, web)
with open('b_1_09', 'wb') as web:
  pickle.dump(b_1, web)
with open('A_2_09', 'wb') as web:
  pickle.dump(A_2, web)
with open('b_2_09', 'wb') as web:
  pickle.dump(b_2, web)
with open('A_3_09', 'wb') as web:
  pickle.dump(A_3, web)
with open('b_3_09', 'wb') as web:
  pickle.dump(b_3, web)
with open('A_4_09', 'wb') as web:
  pickle.dump(A_4, web)
with open('b_4_09', 'wb') as web:
  pickle.dump(b_4, web)
